...
/*   Updated: 2022/11/11 15:04:02 by aait-mal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_option(const char *string, va_list arg, int *length)
{
	...
	if (*string == 'x')
		ft_putnbr_base(va_arg(arg, int), BASE_HEX_LOW, length);
	if (*string == 'X')
		ft_putnbr_base(va_arg(arg, unsigned int), BASE_HEX_UP, length);
	if (*string == 'p')
	{
		ft_putstr_fd("0x", 1, length);
		ft_putptr(va_arg(arg, int *), length);
	}
}
